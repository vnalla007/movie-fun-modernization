...
public class AlbumsController {
    ...
    public HttpEntity<byte[]> getCover(@PathVariable long albumId) throws IOException {
        ...
        return makeHttpEntityFromBlob(coverBlob);
    }

    private HttpEntity<byte[]> makeHttpEntityFromBlob(Blob coverBlob) throws IOException {
    ...
    public HttpEntity<byte[]> getThumbnail(@PathVariable long albumId) throws IOException, InterruptedException {
        Blob thumbnailBlob = coverCatalog.getThumbnail(albumId);

        return makeHttpEntityFromBlob(thumbnailBlob);
    }
}
